...
const (
	...
	//
	// Deliberately higher than DefaultBrokerDockerDiskUsageThresholdPercent:
	// broker hosts routinely idle above 80% (the reference host sits at 83%
	// with 150 GB free), and at 80% every unreferenced template would be
	// evicted on every pass, so the cache would never be warm. Templates are
	// also the wrong thing to give up first -- one is 1-2 GB, where the Docker
	// image store is tens of GB.
