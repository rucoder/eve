...
func TestClusterToSingleConversion(test *testing.T) {
	...
	for _, name := range remainingNames {
		...
		t.Eventually(updates, remainingTimeout).Should(Receive(
			matchers.SatisfyPredicate(
				fmt.Sprintf("Device %q no longer reports the converted node", name),
				func(info *eveinfo.ZInfoKubeCluster) bool {
					for _, n := range nodeNamesReported(info) {
						if n == convertedName {
							return false
						}
					}
