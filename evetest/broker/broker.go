...
func (b *broker) BuildImage(
	...
	// at all -- that is the point of the feature -- so its preconditions and
	// miss check must be resolved before any docker image I/O is attempted:
	// a HaveDockerImage/PullDockerImage below would otherwise waste (or fail)
	// a multi-GB pull that the live path never needed.
	...
	if b.provider.DiskImageStrategy() == provider.DiskImageLegacyBuild {
		...
		// never inspected. The cache key comes from liveImageSHA256 via
		// liveTemplateKeyParams instead; dockerImageID stays empty and unused.
