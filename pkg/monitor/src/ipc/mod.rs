...
// The graphical frontend's pillar client: a shared last-known-state snapshot
// behind a mutex, filled by a background thread. The TUI does not use this -
// its Elm-style loop drives ipc_client::IpcClient directly from
// application.rs - so this is dead code until crate::gui becomes reachable,
// not an oversight.
