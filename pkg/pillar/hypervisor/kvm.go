...
// detectIommuAWBits returns the largest guest address width (39 or 48) that
// every host VT-d unit can actually back. qemu 10.x rejects vfio-pci attach
// when the vIOMMU's aw-bits exceeds the host IOMMU's MGAW, and intel-iommu's
// compile-time default is 48 — which fails on consumer/edge SKUs whose
// physical address width is < 48 (e.g. Raptor Lake = 46 bit).
//
// MGAW lives in cap[21:16] and reports (N-1) where N is the number of guest
// address bits the unit can translate. We pick 48 only if every DMAR has
// MGAW >= 47 (i.e. supports a full 48-bit guest AW); otherwise fall back to
// 39 which is mandatory for VT-d2 and sufficient for 512 GB of guest RAM.
func detectIommuAWBits() int {
	const fallback = 39
	matches, err := filepath.Glob("/sys/class/iommu/dmar*/intel-iommu/cap")
	...
	for _, p := range matches {
		...
		cap, err := strconv.ParseUint(strings.TrimSpace(string(raw)), 16, 64)
		if err != nil {
			return fallback
		}
		mgaw := int((cap>>16)&0x3f) + 1
		if mgaw < 48 {
			return fallback
		}
	}
...
{{end}}
{{- if .DomainConfig.EnableVnc }}
{{- /* Default: virtio-gpu-pci (matches eve-k / kubevirt). */}}
{{- /* Legacy: vnc.driver.legacy=true → virtio-vga (carries a stub VGA */}}
{{- /* controller for BIOS-mode boot but conflicts with iGPU passthrough */}}
{{- /* on q35 — only kept for backward compatibility). */}}
{{- if ne .Machine "virt" }}
[device "video0"]
{{- if .VNCDriverLegacy }}
  driver = "virtio-vga"
...
{{- end}}
{{- if .XPCIClassCode }}
  x-pci-class-code = "{{.XPCIClassCode}}"
{{- end}}
`
...
type tQemuGlobalConfContext struct {
	...
	VNCDriverLegacy        bool   // true when vnc.driver.legacy is set; force legacy virtio-vga instead of default virtio-gpu-pci
	...
	IommuAWBits            int    // intel-iommu aw-bits, derived from host SAGAW; 39 or 48 (qemu 10.x rejects vfio attach if vIOMMU AW exceeds host AW)
	types.DomainConfig
	types.DomainStatus
}
...
type tQemuPCIPassthruContext struct {
	...
	XPCIClassCode string // dev-only override for vfio-pci x-pci-class-code; empty = qemu uses host class. e.g. "0x030000" (VGA) or "0x038000" (Display Other).
	Bus           string
	Addr          string
}
	...
	for _, pa := range pciAssignments {
		...
		if isIntelIGPU {
			...
			if ov := loadIgpuOverrides(); ov != (igpuIDOverride{}) {
				...
				if ov.XPCIClassCode != "" {
					logrus.Warnf("iGPU passthrough [DEV id.json]: applying x-pci-class-code=%q "+
						"for domain %s", ov.XPCIClassCode, f.domainUUID)
					pciPTContext.XPCIClassCode = ov.XPCIClassCode
				}
			}
...
//
// XVGA / XIGDOpRegion / ROMBar / XPCIClassCode — *DEV-ONLY* overrides for
// the qemu vfio-pci device options on the iGPU passthrough device.  Empty
// string or omitted = pillar's default behaviour.  When set, applied
// always (regardless of proprietary-GOP path).  Used for triage of iGPU
...
//	  "x_pci_class_code":  "0x038000"    // dev: "0x030000" (VGA) / "0x038000" (Display Other) / omit=use host class
//	}
//
...
//
// x_pci_class_code: hex 24-bit value flipping the vfio-pci device's
// guest-visible PCI class.  Used to test boot-primary-display-adapter
// behaviour during iGPU passthrough triage.
// 0x030000 = VGA Controller; 0x038000 = Display Other.
type igpuIDOverride struct {
	...
	XPCIClassCode string `json:"x_pci_class_code,omitempty"`
}
...
func loadIgpuOverrides() igpuIDOverride {
	...
	// x_pci_class_code: hex/decimal 24-bit value, normalised to "0x%06x".
	// QEMU rejects values that don't look like a valid PCI class triple.
	if s := strings.TrimSpace(raw.XPCIClassCode); s != "" {
		v, err := strconv.ParseUint(s, 0, 24)
		if err != nil {
			logrus.Warnf("loadIgpuOverrides: %s: bad x_pci_class_code %q: %v", path, s, err)
		} else {
			out.XPCIClassCode = fmt.Sprintf("0x%06x", v)
		}
	}
	return out
}
...
func (ctx KvmContext) CreateDomConfig(domainName string,
	...
	var efiDebug, vncDriverLegacy bool
	var gopRomFilename string
	if globalConfig != nil {
		efiDebug = globalConfig.GlobalValueBool(types.EnableEFIDebug)
		vncDriverLegacy = globalConfig.GlobalValueBool(types.VNCDriverLegacy)
	...
	qemuConfContext := tQemuGlobalConfContext{
		...
		BootOrder:              bootOrder,
		EFIDebug:               efiDebug,
		VNCDriverLegacy:        vncDriverLegacy,
		HasIntelIGPU:           hasIntelIGPU,
		IgpuLpcDeviceID:        igpuLpcID,
		IommuAWBits:            detectIommuAWBits(),
		DomainConfig:           config,
		DomainStatus:           status,
	}
