...
type tQemuGlobalConfContext struct {
	...
	HasIntelIGPU           bool   // true when Intel iGPU is in the passthrough list; moves USB root port to slot 0x1f
	types.DomainConfig
	types.DomainStatus
}
...
func (f *pciAssignmentsTemplateFiller) do(pciAssignments []pciDevice) error {
	...
	for _, pa := range pciAssignments {
		...
		if isIntelIGPU {
			...
			// The USB root port was moved from 0x2 to 0x1f in qemuGlobalConfTemplate.
...
// (as read from sysfs, e.g. "0x3ea0"). Returns empty string if the file does not exist.
func gopRomPath(devid string) string {
	name := strings.TrimPrefix(strings.TrimSpace(devid), "0x")
	path := filepath.Join(gopRomDir, name+".rom")
