		...
		if vendor, err := pa.vid(); err == nil && vendor == "0x8086" {
			if pciPTContext.Xvga {
				...
				if devid, err := pa.devid(); err == nil {
					logrus.Infof("iGPU passthrough: looking for GOP ROM in %s for device ID %s (domain %s)",
						gopRomDir, devid, f.domainUUID)
					pciPTContext.Romfile = gopRomPath(devid)
					if pciPTContext.Romfile != "" {
						logrus.Infof("iGPU passthrough: using GOP ROM %s for domain %s",
							pciPTContext.Romfile, f.domainUUID)
					} else {
						logrus.Infof("iGPU passthrough: no GOP ROM found in %s for device %s, "+
							"using open-source path (no physical display output) for domain %s",
							gopRomDir, devid, f.domainUUID)
					}
				} else {
...
// gopRomDir is where pre-built Intel GOP Option ROMs are stored on the device.
...
// gopRomPath returns the path to the GOP ROM for the given PCI device ID string
// (as read from sysfs, e.g. "0x3ea0"). Checks canonical name first (3ea0.rom),
// then the 0x-prefixed variant (0x3ea0.rom). Returns empty string if neither exists.
