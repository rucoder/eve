...
{{- if .VMTopology.IsSet}}
  sockets = "{{.VMTopology.Sockets}}"
  cores = "{{.VMTopology.Cores}}"
  threads = "{{.VMTopology.Threads}}"
{{- else}}
  sockets = "1"
...
type KvmContext struct {
	...
	// status is bound per-domain by Task(); it carries OrderedCPUs/EmulatorCPUs
	// so Start() can apply topology pinning without any ephemeral hand-off state.
...
func (ctx KvmContext) Task(status *types.DomainStatus) types.Task {
	...
	// Bind the domain's status so Start() can read OrderedCPUs/EmulatorCPUs for
	// topology pinning (value receiver: this sets the field on the returned copy).
	ctx.status = status
	return ctx
}
...
func (ctx KvmContext) Start(domainName string) error {
	...
	var ordered, emulator []uint32
	if ctx.status != nil {
		ordered = ctx.status.OrderedCPUs
		emulator = ctx.status.EmulatorCPUs
	}
	if err := ctx.pinDomainThreads(domainName, qmpFile, ordered, emulator); err != nil {
