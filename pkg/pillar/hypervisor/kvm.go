...
{{- /* No virtual IOMMU when an Intel iGPU is assigned.  i915 does not
       DMA-map stolen memory - it wraps the firmware-reserved physical
       range in what the driver itself calls a "fake dma mapping" and
       puts the physical address straight into the scatterlist.  With a
       vIOMMU present the guest takes over DMA remapping, QEMU drops the
       flat guest-RAM mapping in favour of on-demand mappings driven by
       the guest's own page tables, and those stolen accesses have no
       translation entry.  Every scanout fetch then faults, permanently,
       on a DMAR unit shared with other devices.  QEMU's virtual
       intel-iommu has no RMRR support, so there is no way to express the
       identity mapping real firmware uses to make this work. */}}
{{- if not .HasIntelIGPU }}
...
{{- end -}}
{{end}}
{{- if or .DomainConfig.EnableVnc .VirtualGPU }}
{{- /* virtio-vga keeps VGA compatibility for firmware and guest installers;
       the -gl variants additionally export the scanout as a dmabuf, which is
       what the graphical console imports zero-copy. Without a render node the
       -gl device cannot be created, so fall back to the plain one. */}}
{{- if ne .Machine "virt" }}
[device "video0"]
  driver = "{{if .VirtualGPU}}virtio-vga-gl{{else}}virtio-vga{{end}}"
  bus = "pcie.0"
  addr = "0x1"
{{- else }}
[device "video0"]
  driver = "{{if .VirtualGPU}}virtio-gpu-gl-pci{{else}}virtio-gpu-pci{{end}}"
{{- end }}
{{- end }}
{{- if .HasIntelIGPU }}
...
type tQemuGlobalConfContext struct {
	...
	HasIntelIGPU           bool   // true when Intel iGPU is in the passthrough list; moves USB root port from slot 0x2 to 0x1b and suppresses the virtual IOMMU
	...
	VirtualGPU             bool   // true when this device can back a guest GPU with host GL, so the graphical console has something to show
	types.DomainConfig
	types.DomainStatus
}
...
// hostHasRenderNode reports whether this device can give guests a GL-backed
// virtual GPU.
func hostHasRenderNode() bool {
	_, err := os.Stat(renderNode)
	return err == nil
}
