...
// NewRDTManager creates a new uninitialized RDT Manager.
// Call Init() to initialize the pqos library and discover capabilities.
...
func (m *RDTManager) Init() error {
	...
	if m.l3Cap != nil {
		...
		for _, l3id := range l3CATIDs {
			...
			logFunctionf("RDT L3 domain %d: %d CLOS, %d ways, %d bytes/way, fullMask=0x%x, minCBM=%d",
...
// Fini shuts down the RDT Manager, releasing all resources.
func (m *RDTManager) Fini() {
	...
	m.initialized = false
	logFunctionf("RDT Manager shut down")
}
