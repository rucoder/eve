...
func BuildTopologyFromPQoS(cap *pqos.Capability) (*TopologyInfo, error) {
	...
	l3Groups := map[uint][]int{}
	l3MBAID := map[uint]uint{}
	for _, core := range cpuInfo.Cores {
		l3Groups[core.L3CATID] = append(l3Groups[core.L3CATID], int(core.LCore))
	...
	numaGroups := map[uint][]int{}
	numaL3 := map[uint]map[uint]bool{}
	for _, core := range cpuInfo.Cores {
		numaGroups[core.NUMA] = append(numaGroups[core.NUMA], int(core.LCore))
...
func BuildTopologyFallback(totalCPUs int) *TopologyInfo {
	cores := make([]int, totalCPUs)
...
// or nil if not found.
func (t *TopologyInfo) FindL3DomainForCore(core int) *L3Domain {
...
// or nil if not found.
func (t *TopologyInfo) FindNUMANodeForCore(core int) *NUMANode {
...
func (t *TopologyInfo) IsIsolated(cpu int) bool {
...
func parseIsolatedCPUsFromCmdline(cmdline string) []int {
