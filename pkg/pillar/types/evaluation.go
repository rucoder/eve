...
const (
	// FaultActionNone - No fault injection
	FaultActionNone FaultAction = "none"
	// FaultActionPanic - Trigger kernel panic
	FaultActionPanic FaultAction = "panic"
	// FaultActionReboot - Force immediate reboot
	FaultActionReboot FaultAction = "reboot"
	// FaultActionDelay - Delay boot process
	FaultActionDelay FaultAction = "delay"
)

// FaultConfig represents fault injection configuration
type FaultConfig struct {
	// Action to take when this slot boots
	Action FaultAction `json:"action"`
	// DelaySeconds for delay action (only used with FaultActionDelay)
	DelaySeconds int `json:"delay_seconds,omitempty"`
	// Note for debugging/tracking
	Note string `json:"note,omitempty"`
}

// FaultInjectConfig maps slots to fault configurations
type FaultInjectConfig struct {
	// Slots maps slot names to fault configurations
	Slots map[SlotName]FaultConfig `json:"slots"`
	// Enabled globally enables/disables fault injection
