...
// goImportFor maps a Go type spelling prefix to the import spec it needs (for
// the shadow structs in the generated Go file). Values are full import specs
// (so aliased imports like uuid work).
var goImportFor = map[string]string{
	"netip.": `"net/netip"`,
	"time.":  `"time"`,
	"net.":   `"net"`,
	"uuid.":  `uuid "github.com/satori/go.uuid"`,
}
