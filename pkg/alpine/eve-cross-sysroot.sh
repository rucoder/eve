...
# the target-arch eve-alpine mirror that was COPYed into the build container.
...
apk index --rewrite-arch "$NATIVE_APK_ARCH" \
    -o "$CACHE_DIR/APKINDEX.tar.gz" "$CACHE_DIR"/*.apk

# Prepare /out/ for package installation
rm -rf /out
mkdir -p /out

# Install the requested packages into /out/
# --allow-untrusted: APKINDEX is unsigned (no abuild-sign available)
