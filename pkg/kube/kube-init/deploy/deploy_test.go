...
// TestRunStartsIndependentComponentsInParallel covers the key new-
// runner property: a fast component whose deps are satisfied does
// not wait for a slow unrelated component. Under the previous wave-
// based runner, `fast` would wait for `slow` to finish its wave;
// under the work-queue runner, `fast` starts immediately.
...
// ---------------------------------------------------------------------------
// BestEffort background retry loop (task #7b / design §4.5.4)
// ---------------------------------------------------------------------------
...
func TestBestEffortWithoutRetryCtxIsOneShot(t *testing.T) {
	// RetryCtx=nil (the default) reproduces the previous one-shot
	// log-and-forget behaviour. A failing BestEffort component
